...
import { formatDate, BlogOverview } from "@/app/utils/blogs/blog";
...
export default function BlogAdminClient({ blogItems: initialItems }: { blogItems: BlogOverview[] }) {
  const [blogItems, setBlogItems] = useState(initialItems);
        ...
        {blogItems.map((item) => (
          <div key={item.id} className="w-full">
            <div className="flex flex-col sm:flex-row gap-4 bg-white/90 dark:bg-white/5 border border-zinc-200 dark:border-white/10 backdrop-blur-lg rounded-2xl shadow p-4 h-full justify-between">
              <div className="aspect-2/1 bg-transparent border border-zinc-200 dark:border-white/10 sm:w-[75%] rounded-lg bg-cover bg-center" style={{ backgroundImage: item.image ? `url(${encodeURI(item.image)})` : "none" }} />
              <div className="flex flex-col justify-between w-full gap-2 md:gap-4">
                ...
                <div className="flex gap-2">
                  <Link href={{ pathname: "/admin/posts/edit", query: { id: item.id } }} className="flex w-fit bg-sky-600 items-center justify-center px-4 py-2 rounded-md text-white cursor-pointer font-semibold hover:bg-sky-500">
                    <EditNoteRoundedIcon />
                    Edit
                  </Link>
                  <button onClick={() => handlePopupDelete(item.id)} className="flex w-fit bg-red-600 items-center justify-center px-4 py-2 rounded-md text-white cursor-pointer font-semibold hover:bg-red-500">
                    <DeleteRoundedIcon />
                    Delete
                  </button>
                </div>
