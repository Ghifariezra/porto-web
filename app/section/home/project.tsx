    ...
    <section className="grid grid-rows-1 sm:grid-cols-3 sm:grid-rows-1 px-4 py-10 gap-4 sm:gap-8 bg-zinc-50 dark:bg-zinc-800">
      ...
      <div className="grid lg:grid-cols-2 gap-6 sm:col-span-2">
        {projectCards.map((item, index) => (
          <div
            key={index}
            className="flex flex-col w-full h-fit gap-4 px-6 pt-6 bg-white/90 dark:bg-white/5 border border-zinc-200 dark:border-white/10 backdrop-blur-lg rounded-2xl overflow-hidden group hover:shadow-xl hover:shadow-cyan-200/40 dark:hover:shadow-cyan-400/20 transition-transform duration-300 hover:scale-[1.02]"
          >
            <div className="flex w-full justify-center items-center gap-2">
              <h1 className="font-bold text-center text-sm sm:text-lg px-3 py-1 rounded-full w-fit bg-zinc-800 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-800">{item.status}</h1>
            </div>
            <div className="flex justify-between items-center gap-2">
              <h1 className="font-semibold text-[11px] sm:text-base w-full px-3 py-1 rounded-full border dark:border-none dark:bg-white text-zinc-800">{item.title}</h1>
